...
window.addEventListener('load' , generateCallbacks);

async function audioForImproization(improv_array, fns) {
    ...
    for (var i = 0; i < improv_array.length; i++) {
        ...
        for (var j = 0; j < chord.length; j++) {
            ...
            for (var n = 0; n < beat.length; n++) {
                ...
                await sleep(duration); //Let the note play before going on to play next note
            }
...
/**
 * Sourced from link below, contains frequencies in Hz of all 88 piano notes
